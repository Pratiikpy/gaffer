...
- **Delivered:** a Mollusk harness (`scripts/svm-tests.sh`) driving the kernel against the real txoracle
  binary, its real anchored roots, and real proofs — `settle_no` pays the NO side the pot, and the rake
  takes its basis points from a winner and nothing from a refund.
