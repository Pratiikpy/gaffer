...
- **LATCH kernel** (Anchor / Solana, `HBJKUPdL4g1K7jpJdPMACMDK6nhPc44gd8RaPtHgwhcG`): non-custodial parimutuel pools + N-leg parlays, self-settling via the `validate_stat` CPI. `lock_ts` closes the oracle-latency exploit (no call lands after the cut-off). **38/38 on the devnet test suite** (`npm run test:kernel`, against real anchored proofs) — pro-rata payout to the lamport, refunds, parlay sweeps/busts, every settlement-binding negative, and the rake. Plus 14 kernel unit tests, a Mollusk harness that settles a real proof against the real oracle, and 202 assertions across the app's suites.
